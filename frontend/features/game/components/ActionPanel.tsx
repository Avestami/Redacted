          ...
          {actions.map((action) => (
            <button
              ...
              className={cn(
                ...
                selectedAction === action.id ? "bg-primary/20 border-primary ring-1 ring-primary shadow-[0_0_15px_rgba(255,255,255,0.1)]" : "bg-black/40 border-white/5 hover:border-primary/50",
                action.bg
              )}
            >
              ...
              <div className="mt-2">
                <div className={cn("font-bold font-mono text-sm tracking-wider", selectedAction === action.id ? "text-primary text-glow" : "text-foreground")}>{action.label}</div>
                <div className="text-[9px] text-muted-foreground uppercase tracking-wide opacity-70 group-hover:opacity-100 transition-opacity">{action.desc}</div>
              </div>
            </button>
          ))}
        </div>
        <Button className="w-full py-6 text-lg tracking-widest relative overflow-hidden group" onClick={handleExecute} disabled={!selectedAction || (!selectedTargetId && selectedAction !== "intel")} variant={selectedAction === "hack" ? "danger" : "primary"}>
